    ...
    try {
      ...
      const mokaPayload = {
        ...
        payload: {
          ...
          toplamTutar: finalAmount, // send same amount for overall total if testing
          taksitMi: installmentCount > 1,
          fundId: 'fbiad-bagis',
          agreementsAccepted: 1,
          plan: Array.from({ length: installmentCount }).map((_, i) => ({ id: `BAGIS-${Date.now()}-${i}` }))
        }
      };
            ...
            <form onSubmit={handleSubmit} className="space-y-10">
              ...
              <section>
                ...
                <div className="mt-6 space-y-2">
                  <label className="text-sm font-semibold text-gray-700">Test: Taksit Sayısı</label>
                  <input
                    type="number"
                    min="1"
                    max="12"
                    ...
                    className="w-full px-4 py-3 rounded-xl border-2 border-amber-200 bg-amber-50 outline-none focus:border-amber-400 font-bold"
                  />
                  <p className="text-xs text-amber-600">Sadece test amaçlıdır, girilen değer moka-init servisine `installmentCount` olarak gönderilir.</p>
                </div>
              </section>
